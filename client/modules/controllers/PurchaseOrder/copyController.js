...
app.controller('copyPurchaseOrders', function($rootScope, $scope, $state, $http, $stateParams, $window, $cookieStore, $timeout,$location,$anchorScroll, $filter, CARRIERCODES, 
system_currencies, system_settings, STATES, Data, ovcDash, ORDERTYPES, TRANSFERTYPES, ORGANIZATIONSTRUCTURE, $compile, StyleGroupService, jmsData, OVC_CONFIG, toaster, roleConfigService, TreeViewService , $q, Utils , helper, snackbar) {
    $scope.title        = 'Copy';
    ...
    var purorder_id         =   sessionStorage.cpordid;
    $scope.store_datas      =   [];
    $scope.UOM              =   [];
    $scope.UOMvalues        =   [];
    $scope.list             =   [];
    ...
    var locConfig = true;
     Utils.configurations().then(function(configData){
        ...
        if((configData.config_arr && configData.config_arr.validateProductAndLocationMapping && configData.config_arr.validateProductAndLocationMapping.featureValue ==  false) 
            || !configData.config_arr.validateProductAndLocationMapping.featureValue ){
                locConfig = false
        }
        ...
        $scope.action.enableNeedByDate  = ($state.current.name  ==  'ovc.purchaseorder-copy') && configData.config_arr && configData.config_arr.enableNeedByDate &&
        ...
        if(!GroupByStyle && $state.current.name == 'ovc.purchaseorder-copy')
   ...
   $scope.validation      =   function(data,add , submit){
        ...
        var errorArray          =   [];
            ...
            if(add){
                    if((data.prodresult) || (data.styleresult)){
                    }else{
    ...
    if($state.current.name      ==  'ovc.transfer-copy'){
    ...
    function getskuRules(sku) {
        ...
        if ($scope.storeConfig.replenishmentRoundingQty && $scope.storeConfig.replenishmentRoundingQty.featureValue !== $rootScope.noskuRoundvalue && $state.current.name === 'ovc.purchaseorder-copy') {
    ...
    /******Add skus  and skus from style using add button*******/
    ...
    $scope.order_style=function(po_add){
        ...
        angular.element(document.getElementById('cpomatrix'))
    ...
    $scope.getmodifiedsku = function(styleskus,upload){
        ...
        ovcDash.post(url,dataSet).then(function (data) {
            if(data && data.status != 'error'){
                ...
                else if($state.current.name      ==  'ovc.transfer-copy') {
                    var storeSkuData = {};
                ...
                var output = {
                    "status": "error",
                    "message": "SKU's not available for this store."
                };
                Data.toast(output);
    ...
    $scope.addnewskus=function(newskus,upload,stostore){
        ...
        priceService.then(function(skus){
            ...
            console.log('Price Service Fails :' + error);
        });
    }
    ...
    $scope.getstore_details = function(shipstore,type) {
        ...
        if($scope.config.organization_name == $scope.organization_struct['2'] && $state.current.name == 'ovc.transfer-copy'){
             ovcDash.get('apis/ang_getfromstore?locid=' + encodeURIComponent(storesel)).then(function(results) {
                if((results.status !='error') && (results != undefined) ){
                        $scope.fromstore_datas =results;
                            if((results.length == 1) && ($state.current.name == 'ovc.transfer-copy')){
        ...
        }if($scope.config.organization_name == $scope.organization_struct['1'] && $state.current.name == 'ovc.transfer-copy'){
                ...
                $.confirm({
                    ...
                    confirm: function() {
                        ...
                        Data.delete('/orderitem/' + orditem_id + paginationData).then(function(data) {
                            var output = {"status": "success", "message": "Product  Deleted Successfully"};
                            Data.toast(output);
                            if($scope.withPagination){
                                SKUListUpdate(data);
                            }else{
    ...
    $scope.calcItems1 = function(pindex, qty, cost, ptotal, prcode, style) {
       ...
       var to_tal = parseFloat(quantity * eachcost).toFixed(2);
        var loc = ($scope.po_add.markforstore != undefined && $scope.po_add.markforstore != '') ? $scope.po_add.markforstore : $scope.shippingstr;
    ...
    function hierarchyLocationTransfer(callback){
    ...
    $scope.orderFunctionData    =   function(){
        if($scope.withPagination){
            var obj     =   {data:{purchaseOrderNumber : sessionStorage.copyOrderNumber ,page_lmt:$scope.POform.entryLimit}};
            Data.post('/copyorder', obj).then(function(orderData){
                if(!orderData.error && orderData){
                    purorder_id = orderData.order_data._id;
            ...
            Data.get('/order?id=' + purorder_id).then(function(dataOrder) {
            resultData(dataOrder);
        });
        }
        ...
        function resultData(data){
            ...
            if (porder.purchaseOrderType == 'IBT_M') {
                $scope.vendorhide = true;
                $scope.orderfmstore = false;
            ...
            var storesel=porder.shipToLocation;
                ...
                if($scope.config.organization_name == $scope.organization_struct['1'] && $state.current.name == 'ovc.transfer-copy'){
                    hierarchyLocationTransfer(function(){
                        var allLocations = angular.copy($scope.CorparateLocationData);
                        allLocations = allLocations.filter(
                        function(value){
                            return value.id !== storesel;
                        ...
                        angular.forEach($scope.CorparateLocation,function(item){
                            if(item.id == $scope.po_add.orderstore){
                                $scope.fromstr = item.displayName;
                            }
                        });
            ...
            $scope.O_add = {
                billtostore: porder.billTo,
                shipmethod: porder.shippingMethod,
                shipping_address: porder.FromAddress,
                shiptostore: porder.shipToLocation,
                //billtostore:porder.shipToLocation,
                markforstore: porder.markForLocation,
                address: porder.shipToAddress,
                shipping_special: porder.specialInstructions,
                purorderno: porder.purchaseOrderNumber,
                street_name: porder.toStreetName,
                street_number: porder.toStreetNumber,
                shipping_city: porder.toCity,
                shipping_zipcode: porder.toPostalCode,
                shipping_phone: porder.toPhoneNumber,
                shipping_contact: porder.contactName
            };

            if($scope.allAfs && $scope.allAfs.hierarchy){
                var listafs = [];
                angular.forEach($scope.allAfs.hierarchy, function(value) {
                    if (value.id == $scope.O_add.billtostore) {
                        var newafsobj = {
                            AfsName: value.name,
                            AfsId: value.id
                        }
                ...
                if($scope.afsDatas){
                    if($scope.afsDatas[0])
                     $scope.O_add.billtostore = $scope.afsDatas[0].AfsName ? $scope.afsDatas[0].AfsName : $scope.O_add.billtostore;

                    //$scope.O_add.billtostore = $scope.O_add.billtostore;
                }
            ...
            // $scope.allsubtotal=porder.totalPoCost;
    ...
    /*****Submit of Copy Order****/
    $scope.order_update = function(po_add, oadd , status , valStatus) {
        if($scope.validation(po_add , 0 , valStatus)){
            // var ponumber = sessionStorage.copordernu÷m;
        // var ordid = sessionStorage.coorderid;
        ...
        if (!status) {
            ...
            submitjson.purchaseOrder.needByDate                 =   oadd && oadd.need_by_date ? new Date(oadd.need_by_date).getTime() : "";
        ...
        if($scope.action.enableNeedByDate && $state.current.name  ==  'ovc.purchaseorder-copy' && oadd.need_by_date){
        ...
        if(!$scope.withPagination){    
            Data.put('/order', {
                ...
                if ((results.__v == 0) && (results.__v != undefined)) {
                    ...
                    sessionStorage.copordernum = porderno;
                    sessionStorage.coorderid = orderid;
                    var prodlist = [];
                    var productorders = $scope.list;
                    var shiptoloc = ship_contact = '';
                    shiptoloc = oadd && po_add.shiptostore ? po_add.shiptostore : "";
                    ship_contact = oadd.shipping_contact ? oadd.shipping_contact : "";
                    var putArr  = [];
                    var postArr = [];
                    var qtyArr  = [];
                    var trancreate = true;
                    ...
                    if (status)
                        add_success = ($state.current.name == 'ovc.transfer-copy') ? "Transfer Saved as Draft" : "Order Saved as Draft";
                    ...
                    if (qtyArr.length) {
                        Data.put('/createqtystatus', { data: { arrData: qtyArr } }).then(function(result) {
                            ...
                            Data.toast(output);
                            ($state.current.name      ==  'ovc.transfer-copy') ? $state.go('ovc.ibt') : $state.go('ovc.purchaseorder-list');
                        });
                    }
                }
            });
        }else{
            Data.post('/order/' + purorder_id, { data: newobj }).then(function(results) {
                if (status)
   ...
   //upload file Function
    $scope.uploadOrders         =   function($fileContent,$file){
        ...
        helper.datSplitup($fileContent).then(function(datData){
            var Skus    =   Object.keys(datData).join(',')
            if($scope.withPagination){
                getskuRules(Skus).then(function(data){
                    // if(!data){
                    //     $scope.skuRoundvalue    =   {};
                    // }
                    // _.each(datData, function(value , key){
                    //     var sku={sku:key, qty:value};
                    //     if($scope.checkReorderqty(sku)){
                    //        datData[key]= $scope.changeToBaseqty(sku);
                    //     }
                    // });
    ...
    $scope.SKUQTYChange     =   function(skuData){
        var tempobj = {"item_data":[{
                ...
                "storeId" : $scope.POform.shiptostore,
